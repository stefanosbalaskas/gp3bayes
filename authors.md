# Authors and Citation

## Authors

- **Stefanos Balaskas**. Author, maintainer, copyright holder.
  [](https://orcid.org/0000-0003-2444-9796)

## Citation

Source:
[`inst/CITATION`](https://github.com/stefanosbalaskas/gp3bayes/blob/v0.5.0/inst/CITATION)

Balaskas S (2026). *gp3bayes: Contract-First Bayesian Workflows for
Hierarchical Behavioural Data*. R package version 0.5.0,
<https://stefanosbalaskas.github.io/gp3bayes/>.

    @Manual{,
      title = {gp3bayes: Contract-First Bayesian Workflows for Hierarchical Behavioural Data},
      author = {Stefanos Balaskas},
      year = {2026},
      note = {R package version 0.5.0},
      url = {https://stefanosbalaskas.github.io/gp3bayes/},
    }
